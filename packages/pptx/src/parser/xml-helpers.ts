...
/**
 * Walk siblings, extracting only true element entries.
 */
...
export function readRootAttrs(entry: Record<string, unknown>): Record<string, string> {
  const out: Record<string, string> = {};
  const a = entry[ATTR_KEY];
  if (!a || typeof a !== "object") return out;
  for (const [k, v] of Object.entries(a as Record<string, unknown>)) {
    const name = k.startsWith(ATTR_PREFIX) ? k.slice(ATTR_PREFIX.length) : k;
    out[name] = String(v);
  }
  return out;
}

export function readRawAttrs(entry: Record<string, unknown>): Record<string, string> {
  const out: Record<string, string> = {};
  const a = entry[ATTR_KEY];
  if (!a || typeof a !== "object") return out;
  for (const [k, v] of Object.entries(a as Record<string, unknown>)) {
