 ...
 * catalogue entry.
 *
 * Notes on `commandSchema`:
 *   - "catalogue-args": Generated MCP/CLI adapters can build the
 *     payload directly from this descriptor's args + buildPayload.
 *   - "custom": Hand-rolled adapters retain ownership (custom batch
 *     logic, stdin handling, multi-step pipelines). The catalogue
 *     still owns labels, descriptions, and explicit callability.
 *   - "none": Non-bus reads or editor-only affordances.
 */
