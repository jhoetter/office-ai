...
4. **Adjacency to what we already have.** Does it pull users deeper into our existing DOCX/XLSX/PPTX/PDF surface area? E.g. opening a scanned invoice image directly in our PDF editor.
...
### 1. Images — `.png`, `.jpg`/`.jpeg`, `.heic`/`.heif`, `.webp`, `.tiff`/`.tif`, `.gif`, `.svg`, `.bmp`
...
### 2. Plain text family — `.txt`, `.md`, `.markdown`, `.rtf`, `.rst`, `.adoc`
...
**Why now.** Trivial cost, high "this is my new Notion / my new Obsidian / my new Granola dump" potential. Pairs naturally with OCR, projections, and generated deliverables.

---

### 3. Tabular data files — `.csv`, `.tsv`, `.jsonl`, `.ndjson`, `.parquet`
...
### 4. Audio / video — `.mp3`, `.m4a`, `.wav`, `.ogg`, `.flac`, `.mp4`, `.mov`, `.webm`, `.mkv`
...
- Attachment-aware import flows that can route `.m4a` voice memos from supported storage integrations.

**Why now-ish.** Higher infra cost (GPU / transcription bills), but the differentiation is enormous. Likely a paid premium tier.

---

### 5. Calendar & contacts — `.ics`, `.ifb`, `.vcf`
...
### 6. Archives — `.zip`, `.7z`, `.rar`, `.tar`, `.tar.gz`, `.tgz`
...
### 7. CAD & engineering drawings — `.dwg`, `.dxf`, `.dwf`, `.stp`/`.step`, `.iges`, `.stl`, `.3mf`
...
### 8. Design files — `.psd`, `.ai`, `.indd`, `.fig`, `.sketch`, `.xd`, `.afdesign`, `.afphoto`
...
### 9. Geospatial — `.kml`, `.kmz`, `.geojson`, `.gpx`, `.shp`, `.gpkg`
...
### 10. Code & config — `.py`, `.ts`, `.js`, `.go`, `.rs`, `.java`, `.sql`, `.yaml`, `.toml`, `.json`, `.env`, `.ipynb`
...
### 12. Accounting & banking exchange — `.qbo`, `.qfx`, `.ofx`, `.iif`, `.csv`-as-bank-export, `.xml` (UBL invoices, ZUGFeRD/Factur-X hybrid PDFs), `.edi`
...
- **Email container formats** (`.eml`, `.msg`, `.mbox`, `.pst`) — important to business workflows, but not an `office-ai` document format. Intake, thread state, and sending belong in Cloud / Research surfaces; `office-ai` should only handle extracted document attachments after a host integration routes them in.
- **Legacy Microsoft formats older than `.docx`/`.xlsx`/`.pptx`** (`.wpd`, `.pub`, `.mdb`) — long tail, low frequency, high parser pain. Convert-on-import only.
...
1. **Images with OCR + AI captioning** — broad, daily, AI-leveraged, cheap-ish. _This is the next format package._
2. **Markdown / plain text** — trivial cost, big completeness perception.
3. **CSV / TSV / JSONL** — natural extension of XLSX, closes a real pain.
4. **Audio / video with transcription** — likely a paid premium tier; a wedge product on its own.
5. **Archives + Calendar / Contacts** — quality-of-life, builds trust.
6. **Vertical bets** (CAD, design, geospatial, accounting/e-invoicing) — driven by the first paying beachhead in each vertical, not speculatively.
7. **Trust-building viewers** (HTML, XML, logs, EPUB, fonts) — bundled in throughout, never as a standalone milestone.
