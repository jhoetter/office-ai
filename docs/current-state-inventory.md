...
| -------------------------------------------- | ------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------ |
| `packages/core`                              | Command bus, plugin registry, OOXML utilities, action types.              | `CommandLite`, mutation/diff types, action catalogue contracts, architecture gate.                                | Action metadata does not yet model `agentCallable`, `webCallable`, `cliCallable`, review/dry-run/diff explicitly.                          |
...
| `packages/pptx`                              | PPTX parser/model/serializer, commands, agent API, SVG/HTML renderer.     | 60 command handlers, 80 catalogue entries, real and synthetic fixtures, web editor/present E2E tests.             | Toolbar surface is narrower than palette/CLI; MCP action auto-binding only covers catalogue entries with CLI surface and payload metadata. |
