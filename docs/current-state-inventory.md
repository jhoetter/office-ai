...
Plan/Preview/Apply command lifecycle backed by a local
session-store/data-dir. The web app can inspect that store through a
path-free sessions API, but it still needs a unified session browser and
...
| -------------------------------------------- | ------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------- |
| `packages/core`                              | Command bus, plugin registry, OOXML utilities, action types.              | `CommandLite`, mutation/diff types, semantic diff normalization, action catalogue contracts, command lifecycle envelope/tests, architecture gate.                                                                                                                                                                 | Lifecycle helpers are consumed by MCP; web surfaces do not yet route Plan/Preview/Apply through them uniformly.                               |
...
| `packages/agent`                             | MCP server and CLI wrapper.                                               | `office-agent mcp`, local data-dir session store behind a `SessionStorageAdapter`, schema inspection/migration/cleanup, canonical session/document MCP tools, cross-format Plan/Preview/Apply tools, review tools, audit-log/activity tools, format subcommands, action-to-CLI and action-to-MCP adapters, tests. | Pending-review replay after a full restart is explicit-not-supported; review must happen while the live mutation stack exists.                |
...
- canonical command lifecycle tools:
  ...
  `reject_change`.
- review-safe semantic diffs:
  `office-ai/semantic-diff@1` summaries, anchors, risks and fallback
  diagnostics for command preview/apply/pending-change payloads.
- path-free activity:
  `list_activity` exposes `office-ai/audit-log-entry@1` provenance,
  diagnostics, semantic diff summaries and export command basis without
  leaking local file paths.
- format-specific load tools create compatibility handles:
...
- MCP responses now share canonical document IDs, diagnostics and
  command lifecycle envelopes. Pending metadata survives restart, but
  approve/reject replay of a live pending mutation stack remains open.
...
1. Pending-review metadata survives restart, but replaying the exact
   live pending mutation stack for approve/reject after restart remains
   open.
...
4. Web editor parity is strong per format but weak at the cross-document
   product layer.
5. Storage now has a local adapter boundary, but no non-local adapter is
   implemented yet.
6. Several docs and comments still describe specific embedding or release
   environments instead of an optional adapter boundary.
7. Runtime prereqs are now discoverable through `office-agent doctor`,
   but repair automation remains intentionally manual.

## Fastest wins

1. Add a web session browser and pending-changes panel backed by the same
   session state.
