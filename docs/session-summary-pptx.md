...
- **Exit / motion-path / emphasis animations** — only entrance animations are typed in F4. If a slide's typed animations are *edited*, any unmodeled `<p:timing>` children on that slide are dropped. Slides whose animations are *not* edited continue to round-trip the original `<p:timing>` byte-for-byte.
