...
import { commentPlainText, commentThreads, type CommentThread } from "@/lib/format-helpers";
import type { DocxComment, DocxSnapshot } from "@officeai/docx";

export interface CommentsSidebarProps {
  snapshot: DocxSnapshot | null;
  /** Scroll editor to and visually flash the highlight for this comment id. */
...
/**
 * Threaded comments sidebar. Lists every top-level comment in document
 * order; replies are nested under their parent. Each thread exposes
 * scroll-to-highlight (clicking the comment header), inline reply input,
...
export function CommentsSidebar(props: CommentsSidebarProps): ReactNode {
  const threads = props.snapshot ? commentThreads(props.snapshot) : [];
  return (
    <div className="comments-sidebar flex min-h-0 flex-col gap-2" data-testid="comments-sidebar">
      ...
      {threads.length === 0 ? (
        <p className="text-xs text-secondary">
          No comments yet. Select text and click the comment button in the toolbar.
        </p>
      ) : (
...
function CommentRow(props: {
  ...
  const text = commentPlainText(props.comment);
  return (
