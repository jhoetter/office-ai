/**
 * Minimal synthetic .pptx generated in-browser as a demo "open file"
 * fallback for the PPTX editor route. The package is the smallest valid
 * presentation we can hand-craft: one slide, one master, one layout, one
 * theme, and the wiring relationships in between.
 *
 * Verified by piping the output through `@officeai/pptx`'s parser +
 * serializer in the headless test suite (see fixtures/pptx/synthetic for
 * a richer set built by `scripts/generate-pptx-fixtures.mjs`).
 */
  ...
  <p:cSld>
    <p:spTree>
      ...
      <p:sp>
        ...
        <p:spPr>
          <a:xfrm><a:off x="838200" y="1825625"/><a:ext cx="7468742" cy="1389742"/></a:xfrm>
      ...
      <p:sp>
        ...
        <p:spPr>
          <a:xfrm><a:off x="838200" y="3373437"/><a:ext cx="7468742" cy="1389742"/></a:xfrm>
          <a:prstGeom prst="rect"><a:avLst/></a:prstGeom>
        </p:spPr>
        <p:txBody>
          <a:bodyPr/>
          <a:lstStyle/>
          <a:p><a:r><a:rPr lang="en-US" sz="2400"/><a:t>An AI-native PPTX editor demo. Drag, double-click to edit, ask the agent.</a:t></a:r></a:p>
        </p:txBody>
      </p:sp>
    </p:spTree>
  </p:cSld>
...
export async function buildSamplePptx(): Promise<ArrayBuffer> {
  const z = new JSZip();
  ...
  z.file("ppt/slides/slide1.xml", SLIDE_XML);
