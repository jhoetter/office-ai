 ...
 * LLM bridge route.
 *
 * POST /api/llm — body: { prompt: string; snapshotMarkdown: string }
 ...
 *    response as a signal to fall back to the offline demo recipe so the
 *    e2e specs (and the no-key dev experience) keep working.
 ...
 * model can't invent one. Model defaults to `gpt-4.1` and is overridable
 * via `OPENAI_MODEL` env var.
 */
